      ...
      def resolve(**attributes)
        if attributes 
          if attributes[:targetable_type].present? && attributes[:targetable_id].present?
            like = attributes[:targetable_type].classify.constantize.create(targetable_id: attributes[:targetable_id], targetable_type: attributes[:targetable_type], user_id: context[:current_user].id)
            if like
              { success: "좋아요를 생성했습니다." }
            else
